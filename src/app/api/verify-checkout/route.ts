...
export async function GET(request: NextRequest) {
  try {
    ...
    // Demo mode: handle demo session
    if (sessionId === 'demo_session' || !process.env.STRIPE_SECRET_KEY) {
      const pickupDay = request.nextUrl.searchParams.get('pickup_day') || 'monday'
      const windowStart = request.nextUrl.searchParams.get('window_start') || '08:00'
      const windowEnd = request.nextUrl.searchParams.get('window_end') || '10:00'
